...
INSERT INTO TABLE dwd_price /* 提取省份 */
SELECT
    market,
    CASE
...
-- 计算统计各省份各品种各日的平均价
INSERT OVERWRITE TABLE dwb_price
SELECT
    prvc,
    pz,
...
FROM
    dwd_price
WHERE release_date >= DATE_SUB(CURRENT_DATE, 7)
GROUP BY
    prvc, pz, release_date;

-- 计算各日期全国各品种的平均价格
set hive.exec.dynamic.partition.mode=nonstrict;
INSERT INTO TABLE dwb_price PARTITION (release_date)
SELECT
    '全国' AS prvc,
    pz,
    AVG(average) AS average,
...
-- 创建临时表以存储计算结果
DROP TABLE IF EXISTS ranked_price_rise;
...
SELECT
    ...
    pz,
    price,
    ROUND(rise, 3) AS rise,
    release_date
FROM
    ranked_price_rise
WHERE
    rank <= 5 AND release_date = DATE_SUB(CURRENT_DATE, 1);
...
SELECT
    ...
    rise,
    release_date,
    ROW_NUMBER() OVER (PARTITION BY prvc, release_date ORDER BY rise ASC) AS rank
FROM
    dws_prvc_price_rise;

-- 跌幅
INSERT OVERWRITE TABLE ads_price_fall
SELECT
    prvc,
    pz,
    price,
    ROUND((-rise), 3) AS fall,
    release_date
FROM
    ranked_price_rise
WHERE
    rank <= 5 AND release_date = DATE_SUB(CURRENT_DATE, 1);
-- 历史价格
INSERT OVERWRITE TABLE ads_price
SELECT
    market,
    prvc,
    pz,
    COALESCE(NULLIF(highest, 0), LAG(highest) OVER (PARTITION BY market, prvc, pz ORDER BY release_date)) AS highest,
    COALESCE(NULLIF(lowest, 0), LAG(lowest) OVER (PARTITION BY market, prvc, pz ORDER BY release_date)) AS lowest,
    COALESCE(NULLIF(average, 0), LAG(average) OVER (PARTITION BY market, prvc, pz ORDER BY release_date)) AS average,
    release_date
FROM
    dwd_price
WHERE release_date >= DATE_SUB(CURRENT_DATE, 7);
...
-- pz表
INSERT OVERWRITE TABLE ads_price_pz
SELECT DISTINCT pz
FROM dwd_price;

INSERT OVERWRITE TABLE ads_price_prvcs
    SELECT pz, prvc,ROUND(average, 2) AS average, release_date FROM dwb_price
    WHERE release_date >= DATE_SUB(CURRENT_DATE, 7);